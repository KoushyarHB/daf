...
export function cardListLabel(card: VocabCard): string {
  ...
  const [lemma] = splitHead(head);
  return lemma.trim() || head;
}
...
export function enrichCards(cards: VocabCard[]): EnrichedVocabCard[] {
  ...
  return cards.map((card, index) => {
    ...
    const domId = cardDomId(card, deckNo);
    const created = card.createdAt ?? card.updatedAt;
    const pluralRaw = (card.plural ?? "").trim();
    let pluralLine: string | null = null;
    if (pluralRaw) {
      const frag = canonicalizePluralField(card.head, pluralRaw);
      if (frag) {
        const [lemmaOnly] = splitHead(card.head.trim());
        pluralLine = `${lemmaOnly}, ${frag}`;
      }
    }
    return {
      ...card,
...
export function pluralDiagram(head: string, pluralRaw: string): string | null {
  const frag = canonicalizePluralField(head, pluralRaw);
  if (!frag) return null;
  const [lemmaOnly] = splitHead(head.trim());
  return `${lemmaOnly}, ${frag}`;
}
