import { splitHead } from "@/lib/vocab/card-utils";
...
function HeadBlock({ head }: { head: string }) {
  const headPlain = head.trim();
  const [lemma, ipa] = splitHead(headPlain);
  if (ipa) {
    return (
      <div className="head">
        {lemma}
        <span className="head-ipa text-red-500"> {ipa}</span>
      </div>
    );
  }
  return <div className="head">{headPlain}</div>;
}
